...
if [[ "$2" == "remote" ]]; then
    ...
    if [ "$ARCH" == "amd64" ]; then
        GITHUB_CORE_VERSION="$(json_extract tag_name "$(curl -s 'https://api.github.com/repos/beta-pikonek/pikonek-amd64/releases/latest' 2> /dev/null)")"
    elif [ "$ARCH" == "amd64" ]; then
        GITHUB_CORE_VERSION="$(json_extract tag_name "$(curl -s 'https://api.github.com/repos/beta-pikonek/pikonek-arm64/releases/latest' 2> /dev/null)")"
    else
        GITHUB_CORE_VERSION="$(json_extract tag_name "$(curl -s 'https://api.github.com/repos/beta-pikonek/pikonek-armhf/releases/latest' 2> /dev/null)")"
    fi
    
    echo -n "${GITHUB_CORE_VERSION}" >> "${GITHUB_VERSION_FILE}"
    chmod 644 "${GITHUB_VERSION_FILE}"

    GITHUB_SCRIPT_VERSION="$(json_extract tag_name "$(curl -s 'https://api.github.com/repos/beta-pikonek/pikonek-install/releases/latest' 2> /dev/null)")"
